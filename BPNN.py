    ...
    def TrainEpoch(self, input, target, trainingRate=0.01, momentum=0.5):
        ...
        for index in reversed(range(self.layerCount)):
            if index == self.layerCount - 1:
                ...
                delta.append(delta_pullback[:-1, :] * self.tFuncs[index](
                    self._layerInput[index], True))
        ...
        for index in range(self.layerCount):
            ...
            if index == 0:
                ...
                layerOutput = np.vstack([self._layerOutput[index - 1], np.ones(
                    [1, self._layerOutput[index - 1].shape[1]])])

            curWeightDelta = np.sum( \
                layerOutput[None, :, :].transpose(2, 0, 1) * delta[delta_index][
                                                             None, :,
                                                             :].transpose(2, 1,
                                                                          0) \
                , axis=0)
...
if __name__ == "__main__":
    ...
    for i in range(lnMax + 1):
        ...
        if i % 1000 == 0 and i > 0:
            print(
            "Iteration {0:6d}K - Error: {1:0.6f}".format(int(i / 1000), err))
        if err <= lnErr:
